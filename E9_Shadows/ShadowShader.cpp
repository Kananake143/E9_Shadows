...
void ShadowShader::setShaderParameters(ID3D11DeviceContext* deviceContext, const XMMATRIX &worldMatrix, const XMMATRIX &viewMatrix, const XMMATRIX &projectionMatrix, ID3D11ShaderResourceView* texture, ID3D11ShaderResourceView*depthMap,ID3D11ShaderResourceView* depthMap2, Light* light, Light* light2)
{
	...
	lightPtr->light1.ambient =light->getAmbientColour();
	lightPtr->light1.diffuse = light->getDiffuseColour();
	lightPtr->light1.direction = light->getDirection();
	lightPtr->light1.padding = 0.f;
	deviceContext->Unmap(lightBuffer, 0);
	deviceContext->PSSetConstantBuffers(0, 1, &lightBuffer);

	lightPtr->light2.ambient = light2->getAmbientColour();
	lightPtr->light2.diffuse = light2->getDiffuseColour();
	lightPtr->light2.direction = light2->getDirection();
	lightPtr->light2.padding = 0.f;
